...
export default function createVirtualGrid(options) {
    ...
    _renderCells: function(bounds) {
      ...
      for (var i = cells.length - 1; i >= 0; i--) {
        ...
        if (this._loadedCells.indexOf(cell.id) === -1) {
          this._loadCell(cell, i)
          this._loadedCells.push(cell.id)
        }
      }
    },
    _loadCell(cell, index) {
      setTimeout(
        this.addLayer.bind(this, L.rectangle(cell.bounds, this.options.style)),
        this.options.delayFactor * index
      )
    },
